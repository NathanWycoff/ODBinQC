...
## Randomly shift the mean and observe each method's performance during Phase II
...
for (sig in sigs) {
    ...
    for (d in 1:length(deltas)) {
        ...
        for (iter in 1:iters) {
            ...
            if (iter == iters) {
                pdf(paste('./images/type_2_diff_', rho_oc - rho_ic, '_sig_', sig/N.mu, '.pdf', sep = ''))
                par(mfrow=c(2,2))
                for (chart in charts) {
                    plot(chart, xs, ns)
                }
    ...
    capture.output(print(ARL_profile), file = paste('./output/type_2_arlprofile_sig_', sig / N.mu, '.tex', sep = '')
}
